...
const tooltip_icon_rule =
      ...
      if (k in u.option_sets) {
        ...
        if (fresh || u.filter || u.selection_subset) {
          ...
          if (fresh) {
            ...
            u.id in site.url_options ? u.set(site.url_options[u.id]) : u.state in u.values ? u.set(u.state) : u.reset()
          }
  ...
  elements = {
      ...
      init: function (o) {
        ...
        o.close = function (e) {
          if (this.expanded && (!e || !e.target.classList || !e.target.classList.contains('combobox-component'))) {
            if ('' === this.selection.innerText && this.source.length) this.selection.innerText = this.cleared_selection
                ...
                if ('summary' in p.value.parsed) {
                  ...
                  _u._base_filter.c.forEach(f => {
                    ...
                    if (f.active && checked) {
                      ...
                      s.appendChild(ss)
                      ss.appendChild(document.createElement('span'))
                      ss.lastElementChild.innerText = '(' + (f.value_source ? f.value_source + '; ' : '') + result + ')'
                      e.appendChild(s)
                    }
                  })
        ...
        if (!pass) {
          ...
          if (this.table) {
            ...
            if (this.parsed.time > -1 && this.header.length > 1 && v.time_range.filtered_index) {
              const offset = (v.time_range.filtered_index ? v.time_range.filtered_index[0] : 0) - v.time_range.index[0]
        ...
        if (null !== time && view.valid && var_info && this.view in var_info) {
          ...
          if (summary && y < summary.n.length) {
            ...
            if (site.settings.color_by_order || 'none' === site.settings.color_scale_center) {
              ...
              s.lastElementChild.style.width = 100 - summary['norm_' + site.settings.color_scale_center][y] * 100 + '%'
            }
          }
        }
      },
    ...
    init_input: function (e) {
      ...
      o.wrapper = o.e.parentElement.classList.contains('wrapper') ? o.e.parentElement : o.e.parentElement.parentElement
      if (o.wrapper) {
...
Object.keys(palettes).forEach(k => {
  ...
  if ('continuous-polynomial' === p.type) {
    ...
    for (let i = 0; i < n; i++) {
      const v = i / n
      p.colors.push('rgb(' + poly_channel(0, v, c) + ', ' + poly_channel(1, v, c) + ', ' + poly_channel(2, v, c) + ')')
    }
  }
...
function content_resize(e, full) {
  ...
  if (!full) {
    ...
    f.style.bottom =
      ...
      (!page.bottom_menu || 'closed' === page.bottom_menu.state ? 0 : page.bottom_menu.getBoundingClientRect().height) +
      'px'
    f.style.left =
  ...
  if (site.data.inited.first && k !== site.state) {
    ...
    Object.keys(site.url_options).forEach(s => {
      if (patterns.embed_setting.test(s))
        k +=
          '&' + s + '=' + ('navcolor' === s ? site.url_options[s].replace(patterns.hashes, '%23') : site.url_options[s])
    })
