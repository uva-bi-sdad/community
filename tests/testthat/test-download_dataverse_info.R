...
test_that("downloads and saves the file appropriately", {
  ...
  expect_identical(jsonlite::read_json(paste0(tempdir(), "/1018130V3SWK71R.json")), meta)
})
...
test_that("works with github id", {
  ...
  expect_identical(jsonlite::read_json(paste0(tempdir(), "/1018130V3GMIVZZ.json")), meta)
})
...
test_that("unpublished sets are accessible with a key", {
  ...
  expect_identical(jsonlite::read_json(paste0(tempdir(), "/1018130V3GO64UN.json")), meta)
})
