...
these should at least have columns for `id` (default is `geoid`) and `value` (default is `value`).
...
and a collected measures info file containing information found about any of the included variables found.
...
Such a script can be set as the view's `run_after` script, so after the datasets get rebuilt,
