...
cook_suburbs[, c("ALAND", "AWATER")] <- colSums(st_drop_geometry(cook_county[-1, c("ALAND", "AWATER")]))
st_geometry(cook_suburbs) <- st_union(cook_county[-1,])

shapes[shapes$NAME == "De Witt", "NAME"] <- "dewitt"
shapes <- rbind(shapes[shapes$NAME != "Cook",], chicago, cook_suburbs)
...
data_sex <- read.csv("https://icjia.illinois.gov/arrestexplorer/api/?split=county,gender")
data_age <- read.csv("https://icjia.illinois.gov/arrestexplorer/api/?split=county,age_group")
data_race <- read.csv("https://icjia.illinois.gov/arrestexplorer/api/?split=county,race")
...
input_dataview("main_view", "selected_variable", time = "Year", time_agg = "selected_year")
...
output_map(
  c(data = paste0(dir, "/docs/counties.geojson")),
  id = "main_map", dataview = "main_view", subto = "main_plot"
)
output_legend()
page_tabgroup(
  list(
    ...
    output_plot(
      ...
      dataview = "main_view", subto = "main_map"
    )
  ),
  list(
    name = "Table",
