...
#' those inputs to as values, or the name of a function to trigger. Available function are \code{refresh} (to force
#' a reprocessing of the current input state), \code{reset_selection} (to reset all input elements that are not
#' settings), and \code{reset_storage} (to clear local storage, reset all settings, and reload). This can also be a
#' URL (including protocol) for a button-like link.
