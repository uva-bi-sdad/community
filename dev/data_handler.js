...
function DataHandler(settings, defaults, data, hooks) {
  ...
  data = data || {};
  if ("string" === typeof settings.metadata.datasets)
    settings.metadata.datasets = [settings.metadata.datasets];
  this.map_variables();
...
function passes_filter(entity, time_range, filter, variables) {
  ...
  for (let i = filter.conditions.length; i--; ) {
    ...
    if (
      !(co.time_component
        ? co.check(entity.data[variables[co.name].code], adjs[co.name] || 0)
        : co.check(s[co.name]))
    )
      return false;
  }
          ...
          if (vc in entity.data) {
            ...
            for (let y = time_range[0]; y < yn; y++) {
              if (y >= range[0] && y <= range[1]) {
                const value =
                  "number" === typeof entity.data[vc]
                    ? entity.data[vc]
                    : entity.data[vc][y - range[0]];
                if (!isNaN(value)) {
                  r +=
                    (r ? "\n" : "") +
                    tr +
...
DataHandler.prototype = {
  ...
  format_value: function (v, int) {
    if (null === v || isNaN(v)) {
      ...
      if (this.settings.settings.digits > 0) {
        ...
        return (
          r[0] +
          ("." + (1 === r.length ? "" : r[1]) + "0000000000").substring(
            0,
            this.settings.settings.digits + 1
          )
        );
  ...
  format_label: function (l) {
    return "string" !== typeof l
      ? ""
      : l in this.variables &&
        this.variables[l].meta &&
        this.variables[l].meta.short_name
      ? this.variables[l].meta.short_name
      : l
          .replace(patterns.seps, " ")
          .replace(patterns.word_start, function (w) {
  ...
  ingest_data: function (d, name) {
    ...
    if (!(name in this.info))
      this.info[name] = { schema: { fields: [] }, ids: [] };
    if ("_meta" in d) {
    ...
    if (
      ...
      (!this.defaults.dataset ||
        name === this.defaults.dataset ||
        site.data.inited.first)
    ) {
      this.load_id_maps();
    } else {
      for (const k in this.loaded)
        if (
          Object.prototype.hasOwnProperty.call(this.loaded, k) &&
          !this.loaded[k]
        )
          return void 0;
      this.load_id_maps();
    ...
    this.data_maps[url].queue.forEach((k) => {
      if (this.info[k].schema.fields.length > field) {
        this.info[k].schema.fields[field].ids = this.data_maps[k] =
          k in this.data_maps[url].resource
          ...
          if (map in this.data_maps) {
            if (this.data_maps[map].retrieved) {
              this.info[k].schema.fields[i].ids = this.data_maps[k] =
                k in this.data_maps[map].resource
                  ? this.data_maps[map].resource[k]
                  : this.data_maps[map].resource;
              this.map_entities(k);
            } else {
              if (-1 === this.data_maps[map].queue.indexOf(k))
                this.data_maps[map].queue.push(k);
            }
              ...
              require("https")
                .get(id.map, (r) => {
                  ...
                  r.on("end", () => {
                    this.ingest_map(
                      JSON.parse(c.join("")),
                      r.req.protocol + "//" + r.req.host + r.req.path,
                      i
                    );
                  });
                })
  ...
  init_summary: function (v, d) {
    if (!this.inited_summary[d + v]) {
      (this.in_browser
        ...
        const ny = (vi.time_range[d][2] =
          vi.time_range[d][1] - vi.time_range[d][0] + 1);
        const m = vi[view],
          c = vi.code;
        if (d in this.sets) {
          var o;
          ...
          const at =
            !n || n > 65535 ? Uint32Array : n > 255 ? Uint16Array : Uint8Array;
  ...
  calculate_summary: async function (measure, view, full) {
    ...
    if (m.state[dataset] !== v.state) {
      const s = v.selection[this.settings.settings.summary_selection],
        ...
        subset =
          v.n_selected[this.settings.settings.summary_selection] !==
          v.n_selected.dataset;
      for (let y = ny; y--; ) {
        mo[y] = subset ? [] : order[y];
        ...
        mo.forEach((o, y) => {
          if (levels) {
            ...
            if (ms.n[y]) {
              ...
              const n = o.length;
              for (
                let i = ms.missing[y], bmd = false, bme = false;
                i < n;
                i++
              ) {
            ...
            if (ms.n[y]) {
              ms.norm_median[y] = ms.range[y]
                ? (ms.median[y] - ms.min[y]) / ms.range[y]
                : ms.median[y];
              if (-1 !== ms.break_median[y]) {
                ms.lower_median_min[y] =
                  ms.norm_median[y] -
                  (o[ms.missing[y]][1] - ms.min[y]) / ms.range[y];
                ms.lower_median_range[y] =
                  ms.norm_median[y] -
                  ((o[ms.break_median[y]][1] - ms.min[y]) / ms.range[y] -
                    ms.lower_median_min[y]);
                ms.upper_median_min[y] =
                  ms.norm_median[y] -
                  (o[ms.break_median[y]][1] - ms.min[y]) / ms.range[y];
                ms.upper_median_range[y] =
                  (o[o.length - 1][1] - ms.min[y]) / ms.range[y] -
                  ms.norm_median[y] -
                  ms.upper_median_min[y];
              }
              ms.norm_mean[y] = ms.range[y]
                ? (ms.mean[y] - ms.min[y]) / ms.range[y]
                : ms.mean[y];
              if (-1 !== ms.break_mean[y]) {
                ...
                ms.lower_mean_range[y] =
                  ms.norm_mean[y] -
                  ((o[ms.break_mean[y]][1] - ms.min[y]) / ms.range[y] -
                    ms.lower_mean_min[y]);
                ms.upper_mean_min[y] =
                  ms.norm_mean[y] -
                  (o[ms.break_mean[y]][1] - ms.min[y]) / ms.range[y];
      ...
      Object.keys(s).forEach((id) => {
        ...
        if ("_meta" !== id) {
          ...
          if (id in this.entities) {
            ...
            if (!("features" in this.entities[id]))
              this.entities[id].features = {};
          } else {
            this.entities[id] = {
              group: g,
              data: si,
  ...
  parse_query: function (q) {
      ...
      Object.keys(q).forEach((k) => {
        if ("include" === k || "exclude" === k || k in f) {
          ...
          const aq = patterns.component.exec(k),
            tf = {
              name: k
                .replace(patterns.greater, ">")
                .replace(patterns.less, "<"),
              component: "mean",
              operator: "=",
          ...
          if (aq) {
            if (-1 !== export_options.filter_components.indexOf(aq[2])) {
              ...
              const i =
                time > 0 && time < this.meta.overall.value.length
                  ? time
                  : this.meta.overall.value.indexOf(time);
              if (-1 !== i) {
          ...
          if (
            patterns.operator_start.test(k) &&
            k[k.length - 1] in this.checks
          ) {
            tf.operator = k[k.length - 1];
            if (("<" === tf.operator || ">" === tf.operator) && !a.length)
              tf.operator += "=";
            ...
            if ("object" === typeof tf.value) {
              ...
              f.time_range =
                "=" === tf.operator
                  ? [i, i]
                  : ">" === tf.operator
                  ? [i, this.meta.overall.value.length - 1]
                  : [0, i];
            }
            if (-1 === f.time_range[0]) f.time_range[0] = 0;
            if (-1 === f.time_range[1])
              f.time_range[1] = this.meta.overall.value.length
                ? this.meta.overall.value.length - 1
                : 0;
          } else if ("dataset" === tf.name) {
            f.dataset = tf;
          } else if (tf.name in this.features) {
            if ("id" === tf.name) {
              tf.value = Array.isArray(tf.value)
                ? tf.value
    ...
    if (!("time_range" in f))
      f.time_range = [
        0,
        this.meta.overall.value.length ? this.meta.overall.value.length - 1 : 0,
      ];
    return f;
  },
  export: async function (query, entities, in_browser) {
    ...
    if (-1 === export_options.file_format.indexOf(query.file_format))
      query.file_format = export_defaults.file_format;
    if (!(query.table_format in row_writers))
      query.table_format = export_defaults.table_format;
    const res = {
        statusCode: 400,
      ...
      vars = [],
      feats =
        query.features || JSON.parse(JSON.stringify(export_defaults.features)),
      rows = [],
      range = [Infinity, -Infinity],
    ...
    for (const k in this.export_checks)
      if (k in query) {
        const r = this.export_checks[k](
          "include" === k ? inc : query[k],
          this.variables
        );
        if (r) {
    ...
    if ("wide" === query.table_format) {
      ...
      rows[0] +=
        ...
        "time" +
        sep +
        ("mixed" === query.table_format ? vars : ["variable", "value"]).join(
          sep
        );
    //console.log(rows[0]);
    Object.keys(entities).forEach((k) => {
      ...
      if (
        ...
        (no_filter ||
          passes_filter(e, query.time_range, query.variables, this.variables))
      ) {
    ...
    res.headers["Content-Type"] =
      "text/" + ("," === sep ? "csv" : "plain") + "; charset=utf-8";
    res.body = rows.join("\n");
    ...
    if (download_data_filename.length > 75) {
      download_data_filename =
        "data_bundle_" + download_data_filename.hashCode();
